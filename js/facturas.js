...
export function init(){
  ...
  $neto  = document.getElementById('factura-neto');
  ...
  document.getElementById('factura-filter-btn')
          .addEventListener('click', onFilter);
  document.getElementById('factura-export-btn')
          .addEventListener('click', exportExcel);
  document.getElementById('factura-autofill-btn')
          .addEventListener('click', autofill);

  document.addEventListener('click', delegate);
}

async function autofill(){ /* igual que antes, no afecta IVA */ }
...
async function onFilter(){
  ...
  const rows=await api(`/facturas?start=${s}&end=${e}`);
  render(rows);
}
...
function render(rows){
  ...
  if(!rows.length){
    ...
    document.getElementById('factura-total-bruto').textContent='0.00';
    document.getElementById('factura-total-neto').textContent='0.00';
    return;
  }
  ...
  document.getElementById('factura-total-bruto').textContent =
     rows.reduce((t,r)=>t+(+r.cantidad_bruta||0),0).toFixed(2);
  document.getElementById('factura-total-neto').textContent =
...
function delegate(e){
  ...
  if(e.target.matches('.edit-factura')){
    ...
    Object.entries({
      ...
      'factura-iva-pct':f.iva_porcentaje,
      'factura-bruto':(+f.cantidad_bruta + +f.iva_importe).toFixed(2),
      'factura-retencion':f.retencion,'factura-empresa':f.nombre_empresa
    }).forEach(([i,v])=>document.getElementById(i).value=v);
    $sub.textContent='Actualizar Factura';
    $can.style.display='inline-block';
    document.getElementById('factura-iva-pct').dispatchEvent(new Event('input'));
  }
}
